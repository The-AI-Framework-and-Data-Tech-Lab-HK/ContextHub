# ContextHub: PostgreSQL 16 → openGauss 7.0.0 SQL 兼容性分析报告 (部分总结来源于GPT-5.4)
...
## 三、openGauss 7.0.0 与PG的兼容性逐项分析

### 风险等级说明

- 🔴 **阻断 (BLOCKER)** — openGauss 语法不支持，必须改写 SQL 否则无法执行
- 🟡 **需适配 (ADAPTATION)** — openGauss 功能支持但语法/扩展名不同，需修改代码
