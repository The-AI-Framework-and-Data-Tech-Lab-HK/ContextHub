...
Disable idempotency for one CLI run:
...
By default, Phase 1 enables idempotency and duplicate commit payloads are skipped.

To allow repeated commits to overwrite/update storage, disable idempotency:
...
```bash
AMC_COMMIT_IDEMPOTENCY_ENABLED=false
```

Important:
- In shell, `AMC_COMMIT_IDEMPOTENCY_ENABLED=false` alone is **not exported**.
- Use one of:
  - `export AMC_COMMIT_IDEMPOTENCY_ENABLED=false`
  - `AMC_COMMIT_IDEMPOTENCY_ENABLED=false amc-commit-trajectory ...`

## 7) Current Phase 1 Behavior for L0/L1 + Dataflow

In current Phase 1:
- L0/L1 summaries are generated by rule-based code in `src/core/commit/summarizer.py`.
